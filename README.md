...
| 행사 목록 | 관리자 집계(실시간) |
|---|---|
| ![행사 목록](docs/screenshots/web-events.jpg) | ![집계](docs/screenshots/web-admin.jpg) |

| 심사위원 채점 | 최종집계표(A4 결재본) |
|---|---|
| ![채점](docs/screenshots/web-judge.jpg) | ![집계표](docs/screenshots/web-print.jpg) |

**안드로이드 앱**([dikafryo/judge-app](https://github.com/dikafryo/judge-app))은 같은 서버에 붙어
오프라인 채점·QR 입장·손가락 서명을 지원합니다.

| 입장 | 목록 | 채점 | 오프라인 | 집계 |
|---|---|---|---|---|
| ![](docs/screenshots/app-1.jpg) | ![](docs/screenshots/app-2.jpg) | ![](docs/screenshots/app-3.jpg) | ![](docs/screenshots/app-4.jpg) | ![](docs/screenshots/app-5.jpg) |

## 사용 방법 (요약)

**주최자**
1. `/events` 에서 행사를 만듭니다. 행사명과 관리 비밀번호만 있으면 됩니다.
2. 기본설정(집계 방식·블라인드·선정 인원·결재란) → 평가항목(1레벨 합계 100점, 2레벨 세부) → 평가대상 → 심사위원을 등록합니다.
   심사위원마다 접속 코드와 QR 카드가 자동으로 만들어집니다.
3. 심사 중에는 **집계** 화면이 5초마다 갱신됩니다. 심사위원별 진행률, 순위, 선정·동점 경고가 보입니다.
4. 끝나면 **최종집계표(A4)**·심사위원별 개별심사표·CSV 를 출력하고 심사를 마감합니다.

**심사위원**
1. 받은 코드로 `judge.sw4u.kr` 에 들어가거나 QR·링크를 엽니다. 회원가입이 없습니다.
2. 대상을 고르고 항목별 점수를 넣은 뒤 저장합니다. 완료 표시와 합계가 바로 보입니다.
3. 화면에 손가락으로 **전자서명**하면 최종집계표 서명란에 실립니다.

체험은 `/demo` 에서 샘플 행사로 해 볼 수 있습니다(읽기 전용).
